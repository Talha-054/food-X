...
export default function Reservation() {
    ...
    validPattern.test(e.target.value)? setIsPhoneNoValid(true) : setIsPhoneNoValid(false)
    ...
    validPattern.test(e.target.value)? setIsPersonsValid(true) : setIsPersonsValid(false)
    ...
    validPattern.test(e.target.value)? setIsDateValid(true) : setIsDateValid(false)
    setIsDateValid(true)
    ...
    validPattern.test(e.target.value)? setIsTimeValid(true) : setIsTimeValid(false)
    setIsTimeValid(true)
    console.log(validPattern.test(e.target.value));
            ...
            className="form w-full p-[1em] pb-[20em] flex flex-col justify-center items-center lg:items-start lg:pb-0 lg:pl-[7em]">
                <div className='flex flex-col w-full lg:w-auto lg:justify-center lg:items-center '>
                    <div className='w-full'>
                        <h2 className='font-body text-[#FFFFFF] text-[36px] lg:text-[50px] mb-8 font-semibold'>Tisch reservieren</h2>
                    </div>
                    <div id="phone" className='flex flex-col gap-2 mb-6 w-full'>
                        <label className='text-[16px] lg:text-[20px] font-body font-semibold text-[#FFFFFF] text-start'>Telefonnummer</label>
                        <input name='phone' onChange={handlePhone} type='text' placeholder='+920312xxxxxx2' className={`${isPhoneNoValid? "text-green-600 font-semibold " :
                             "text-red-600 font-semibold "} w-full  px-2 max-w-[650px] lg:w-[400px] h-[50px] rounded-[5px]`}/>
                    ...
                    <div id="person" className='flex flex-col gap-2 mb-6 w-full'>
                        <label className='text-[16px] lg:text-[20px] font-body font-semibold text-[#FFFFFF] text-start'>Personen</label>
                        <input name='person' onChange={handlePersons} type='text' placeholder='Anzahl der Personen' className={`${isPersonsValid? "text-green-600 font-semibold " 
                            : "text-red-600 font-semibold "} w-full px-2 max-w-[650px] lg:w-[400px] h-[50px] rounded-[5px]`}/>
                    ...
                    <div id="date" className='flex flex-col gap-2 mb-6 w-full'>
                        <label className='text-[16px] lg:text-[20px] font-body font-semibold text-[#FFFFFF] text-start'>Datum</label>
                        <input  name='date' onChange={handleDate} type='date' placeholder='date' className={`${isDateValid? "text-green-600 font-semibold " 
                            : "text-red-600 font-semibold "} w-full px-2 max-w-[650px] lg:w-[400px] h-[50px] rounded-[5px]`}/>
                    ...
                    <div id="time" className='flex flex-col gap-2 mb-4 w-full'>
                        <label className='text-[16px] lg:text-[20px] font-body font-semibold text-[#FFFFFF] text-start'>Uhrzeit</label>
                        <input name='time' onChange={handleTime} type='time' placeholder='Time' className={`${isTimeValid? "text-green-600 font-semibold " 
                            : "text-red-600 font-semibold "} w-full px-2 max-w-[650px] lg:w-[400px] h-[50px] rounded-[5px] `}/>
                    ...
                    <div className='w-full flex justify-start'>
                        <button disabled={formValid? false: true}  onClick={handleFormSubmission} className={`w-[175px] ${formValid? "opacity-100" : "opacity-30"} h-[55px] bg-[#760124] text-[#FFFFFF] font-bold rounded-[5px]`}>Tisch reservieren</button>
                    </div>
                </div>
            </motion.div>
    </>
